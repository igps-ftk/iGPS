...
;-
PRO GEO_LINE_XY_FAR_DISt_signed, xy1,  $ ;e.g. xy1=[100,32]
    xy2, $  ;e.g. xy2=[103,35]
    xy3,   $ ;e.g. xy3=xy1
    distance_orig,   $ ;e.g. distance=100 km
    oxy=oxy
  ...
  distance=distance*abs(xy2[0]-xy1[0])/(xy2[0]-xy1[0])
  
;  if distance lt 0 then begin
;    tmp=xy1
;    xy1=xy2
;    xy2=tmp
;    distance=-1d0*distance
;  endif
;  
  ...
  FOR i=0,nmax-1 DO BEGIN
    ds[i]=MAP_2POINTS(xy3[0],xy3[1],xs[i],ys[i],/meters)*1d-3*(abs(xs[i]-xy3[0])/(xs[i]-xy3[0]))
  ENDFOR
  ;stop
  ...
  IF N_PARAMS() LT 4 THEN BEGIN
    ...
    PLOTS,xy3[0],xy3[1],psym=2,color='0000ff'x,symsize=2
    XYOUTS,xy1[0],xy1[1],'xy1',color='0'x
    XYOUTS,xy2[0],xy2[1],'xy2',color='0'x
    XYOUTS,xy3[0],xy3[1],'xy3',color='0'x
    XYOUTS,oxy[0],oxy[1],'oxy',color='0'x
    OPLOT,[xy3[0],oxy[0]],[xy3[1],oxy[1]],color='ff0000'x,linestyle=2,thick=2
    XYOUTS,(xy3[0]+oxy[0])/2,(xy3[1]+oxy[1])/2, $
