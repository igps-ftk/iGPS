...
dlines(:,nl)=sscanf(tline,'%*s %f %f %f %f %f       %f %f %f %f %f       %f %f %f %f %f       %f %f %f %f %f       %f %f %f %f %f ');
    ...
    ssfun    = @(param,data) sum((data.ydata-fun_2d_screw_dislocation_afterslip(data.xdata,param)).^2);
...
%     options.qcov = tcov; % covariance from the initial fit
    ...
    %   xo = linspace(-300,300)';
    xo=x1';
    yo=fun_2d_screw_dislocation_afterslip(xo,mean(chain));
    %figure(1)
    set(0,'CurrentFigure',h1);
    subplot(2,1,1);
    hold on
    plot(xo,fun_2d_screw_dislocation_afterslip(xo,mean(chain)),'-k')
    ...
    clf
    out = mcmcpred(res,chain,[],xo,'fun_2d_screw_dislocation_afterslip');
    %   out = mcmcpred(res,chain,s2chain,xo,'fun_2d_screw_dislocation_afterslip');
    mcmcpredplot(out);
    hold on
    ...
    fprintf(fido,'* postseismic slip rate: %f +/- %f\n', out_params(6), out_sigs(6));
    fprintf(fido,'* postseismic slip upper depth: %f +/- %f\n', out_params(7), out_sigs(7));
    fprintf(fido,'* postseismic slip lower depth: %f +/- %f\n', out_params(8), out_sigs(8));
    fprintf(fido,'*\n');
