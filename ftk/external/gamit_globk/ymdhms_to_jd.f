...
* MOD TAH 141024: More detail added to invalid month entry
      character*80 invalid_month 
...
* MOD TAH 100716: Print warning if month is wrong
      if( month.le.0 .or. month.gt.12 ) then
         write(invalid_month,110) month, date(1:5)
 110     format('INVALID MONTH: ',i5,' Date passed ',5I5,
     .          '. Month set to 1')
         call report_stat('WARNING','GLOBK','YMDHMS_TO_JD',' ',
     .         invalid_month,month)
         month = 1 
      end if
      day       = date(3)
