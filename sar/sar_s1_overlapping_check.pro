 ...
 PRO SAR_S1_OVERLAPPING_CHECK, file, ofile
   IF N_ELEMENTS(file) EQ 0 THEN BEGIN
     file='D:\gsar\interseismic\t92-d-m_cangmuco_palongco\input.lst'
   ENDIF
   ...
   FOR i=0, N_ELEMENTS(ids_acquition)-1 DO BEGIN
     ...
     IF N_ELEMENTS(pos) LT 2 THEN BEGIN  ;if the number
     ...
     IF N_ELEMENTS(pos) EQ 2 THEN BEGIN  ;if only two scenes found, check whether they are identidal.
       gap_sec=ABS(tmp3[1]-tmp3[0])
       IF gap_sec LT 10 THEN BEGIN
