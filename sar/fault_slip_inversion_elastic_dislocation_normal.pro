FUNCTION Fault_slip_inversion_elastic_dislocation_normal,  $
    ;inputs
  ...
  IF N_ELEMENTS(ftss) EQ 0 THEN ftss=0
...
PRO Fault_slip_inversion_elastic_dislocation_normal, pfile,   $
    theta_in=theta_in, $
    fts_in=fts_in, $
    distmax=distmax, $
    distmin=distmin,  $
  ...
  IF N_PARAMS() LT 1 THEN BEGIN
    ...
    FOR i=0,N_ELEMENTS(fids)-1 DO BEGIN
      ...
      pfile=pfiles[fi]
      PRINT,pfile
      Fault_slip_inversion_elastic_dislocation_normal, pfile, theta_in=theta_in, fts_in=fts_in, distmax=distmax, distmin=distmin,overwrite=1
    ;RETURN
    ENDFOR
    RETURN
  ;
