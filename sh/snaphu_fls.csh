...
if ($2 == 0) then
  ...
  #changed by tianyf/tyf on Mon, Jun 08, 2020  4:04:17 PM
  echo snaphu phase.in `gmt grdinfo -C phase_patch.grd | cut -f 10` -f $sharedir/snaphu/config/snaphu.conf.brief -c corr.in -o unwrap.out -v -s $MCF
  #time snaphu phase.in `gmt grdinfo -C phase_patch.grd | cut -f 10` -f $sharedir/snaphu/config/snaphu.conf.brief -c corr.in -o unwrap.out -v -s $MCF
  echo "time gmtsar_fls.py $1"
  time gmtsar_fls.py $1
else
  sed "s/.*DEFOMAX_CYCLE.*/DEFOMAX_CYCLE  $2/g" $sharedir/snaphu/config/snaphu.conf.brief > snaphu.conf.brief
  echo snaphu phase.in `gmt grdinfo -C phase_patch.grd | cut -f 10` -f snaphu.conf.brief -c corr.in -o unwrap.out -v -d $MCF
  #time snaphu phase.in `gmt grdinfo -C phase_patch.grd | cut -f 10` -f snaphu.conf.brief -c corr.in -o unwrap.out -v -d $MCF
  echo "time gmtsar_fls.py $1"
  time gmtsar_fls.py $1
endif
#
...
#
#if ($2 == 0) then
# + changed by tianyf/tyf on Sun Jun  7 06:17:14 EDT 2020
# +   to detrend the interferograms by default
...
mv -f corr_patch.grd corr_cut.grd #change to such, by tianyf/tyf on Sat May 30 18:57:12 CST 2020
