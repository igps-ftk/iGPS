...
c     --PURPOSE--
c     Download broadcast navigation data from IGS Data Center.
...
c     <<VAR_DEC
      ...
      if (iargc().lt.3) then
c         write(*,*) 'Usage: get_nav YEAR DOY NDAYS SITEFILE [ROOT_DIR]'
         write(*,*) 'Usage: get_nav YEAR|YR DOY NDAYS '
...
c     default only download auto brdc/nav
...
c     The default filename for auto/brdc file is different between SOPAC
c     and CDDIS.
      sites(1)='brdc'
...
c     open command file
         ...
         call idate(today)
         call itime(now)
         write(*,*) dir_log(1:nblen(dir_log)),today,now
         write(file_cmd,703) dir_log(1:nblen(dir_log)),today,now,'.cmd'
 703     format(A,"/get-nav-",I2.2,I2.2,I4,"-",3I2.2,a)
...
c         write(*,*) dir_cur(1:nblen(dir_cur)
 ...
 701        format(A,"/",a4,I3.3,"0.",I2.2,"n.Z")
...
c     no checking now
     ...
     &           site,doy,yr
 706        format(A,"/",I2.2,"n/",a4,I3.3,"0.",I2.2,"n.Z")
c            write(*,*) file(1:nblen(file))
c     ,orbType
c     , year,doy,gpsw,gpsd
c            goto 800
...
c 707           format("/pub/gps/data/daily/",I4.4,"/",I3.3,"/",I2.2,"n")  
 707           format("/gps/data/daily/",I4.4,"/",I3.3,"/",I2.2,"n") 
 ...
 702        format(A4,I3.3,"0.",I2.2,"n.Z")
     ...
     &           ' '//file_local(1:nblen(file_remote))
            nf=nf+1
 801        continue
         enddo
