PRO SAR_SBAS_BLOCK_RATE, path,  $
  ...
  IF N_PARAMS() LT 1 THEN BEGIN
    ...
    path='\\gpsac4\root\g4b\tianyf\m_jiali1\asc_F2\SBAS'
  ...
  IF GETFILENAME(files[0]) EQ 'disp_001_ll.nc' THEN BEGIN
    files=files[1:*]
    nf=nf-1
  ENDIF
  ;stop
  ...
  oNx=nx/sx
  oNy=ny/sy
  olon=lon[INDGEN(oNx)*sx]
  olat=lat[INDGEN(oNy)*sy]
  ...
  ;
  ;FOR i=0,nx-1, sx DO BEGIN
  FOR i=0, oNx-1 DO BEGIN
    indx=i*sx+sx/2
    xo=lon[indx]
    IF xo LT xmin || xo GT xmax THEN CONTINUE
    ;FOR j=0,ny-1, sy DO BEGIN
    FOR j=0, ony-1 DO BEGIN
      indy=j*sy+sy/2
      yo=lat[indy]
      IF yo LE ymin || yo GT ymax THEN CONTINUE
      ...
      count=count+1
      IF count GT 9999 THEN STOP
      
      site_name=STRING(count,format='(i04)')+  $
      ...
      ;STRING(yo,format='("_Y",f07.3)')
      ;
      site_names[count]=site_name
      
      IF out_plot EQ 1 THEN BEGIN
        WSET,0
        XYOUTS,[indx]*sf,[indy]*sf,'+',color='ffffff'x,/device
        XYOUTS,[indx]*sf,[indy]*sf,'  '+STRTRIM(count,2),color='ffffff'x,/device,charsize=.8
      ENDIF
      ...
      oRatesXyz[*,count]=[xo,yo,tmp[1],stdv[1]]
      oRates[i,j]=tmp[1]
      oRMSs[i,j]=stdv[1]
      ...
      oRatesXyz[*,count]=[xo,yo,coef[1],sig[1]]
      oRates[i,j]=coef[1]
      oRMSs[i,j]=sig[1]
      llhs[*,count]=[xo,yo,0]
      ;stop
      ...
      IF out_plot EQ 1 THEN BEGIN
        ...
        XYOUTS,MEAN(xs),30,site_name+STRING(oRatesXyz[2:3,count],format='(1x,"rate=",f7.2,1x,"+/-",f7.2,1x,"mm/yr")'),/data,color='0'x,alignment=.5
        ;PRINT,jfile
