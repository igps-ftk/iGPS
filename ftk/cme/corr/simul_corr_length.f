...
c     read in all apriori coordinates
      do i=1,nf
         site1=sites(i)
         call read_neuxyz(coords_file,site1,xyz1,ll1)
         llh(i,1)=ll1(2)
         llh(i,2)=ll1(1)
         llh(i,3)=ll1(3)
      enddo
...
c               mjd2(k)=data_all(k,1,j)
     ...
     &           nmax_cal.lt.nrows(i).and.nmax_cal.lt.nrows(j)) then
               tmp=corr_xy2(mjd1,ts1,nmax_row,nmax_cal,
     &              mjd2,ts2,nmax_row,nmax_cal,
     &              nsame)
            else
               tmp=corr_xy2(mjd1,ts1,nmax_row,nrows(i),
