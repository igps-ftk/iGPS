...
C     ---
      character*1023 path,opath,ofile,ofilebd,ofilebk,ofileN,tmpstr
      ...
      integer*4 nepochs(nmax_sites,nmax_sites,3),nepoch
      integer*4 fid,fido,fidobd,fidobk,ioerr,fidoN
...
c     
      character*1023 fmtstrN

C     ---
      if (iargc().lt.2) then
         write(*,*) 'Usage: ts_corr input_path output_path'
         ...
         write(*,*) 'Please edit those variables accordingly.'
         stop
      endif
...
c$$$            if (site1.eq.'p455'.and.site2.eq.'smyc') then
c$$$               write(*,*),site1,site2,dist,ll1,ll2
c$$$               stop
c$$$            endif
c            stop
         enddo
...
c     nepoch output
            ofileN=opath(1:nblen(opath))//pathsep//
     &           prext_net(1:nblen(prext_net))//'_'//
     &           prext_org(1:nblen(prext_org))//'_corr_'//
     &           neustr(neui)//'.nepoch'
            call getlun(fidoN)
            open(unit=fidoN,file=ofileN)
...
c$$$      write(fido,fmtstr) ' ',(sites(i),i=1,nf)
      ...
      write(fidoN,fmtstr) ' ',(sites(i),i=1,nf)
      
c     write(fmtstr,'("(a4,1x,",i5,"f11.5)")') nf
      write(fmtstr,'(a,i5,a)') '(a4,1x,',nf,'f12.5)'   
      write(fmtstrN,'(a,i5,a)') '(a4,1x,',nf,'i12)'   
...
c               mjd2(k)=data_all(k,1,j)
               ...
               mjd2(k)=tmpmjd
            enddo
c            tmp=corr_xy(mjd1,ts1,nmax_row,nrows(i),
            tmp=lin_corr_xy(mjd1,ts1,nmax_row,nrows(i),
     &           mjd2,ts2,nmax_row,nrows(j),nepoch)
c            write(*,*) tmp
            mat_corr(i,j)=tmp
            mat_corr(j,i)=tmp
            mat_corrs(i,j,neui)=tmp
            mat_corrs(j,i,neui)=tmp
            nepochs(i,j,neui)=nepoch
            nepochs(j,i,neui)=nepoch
c            write(*,*) site1,' ',site2,':',tmp
c            write(*,*) site1,' ',site2,':',nepoch
c            stop
         enddo         
         write(fido,fmtstr) sites(i),(mat_corr(i,j),j=1,nf)
         write(fidoN,fmtstrN) sites(i),(nepochs(i,j,neui),j=1,nf)
c         write(*,fmtstrN) sites(i),(nepochs(i,j,neui),j=1,nf)
