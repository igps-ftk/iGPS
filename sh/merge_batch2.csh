...
#
  ...
  if ($#argv != 3) then
    echo ""
    echo "Usage: merge_batch.csh inreffile inputfile config_file"
    echo ""
  ...
  set ref_file=$1 
  set input_file = $2
  if (! -f tmpm.filelist ) then
    awk 'NR==1{print $0}' $ref_file | awk -F, '{for (i=1;i<=NF;i++) print "../"$i}' | awk -F: '{print $1$2}'> tmpm.filelist 
  endif
  ...
  foreach line (`awk '{print $0}' $input_file`)
    ...
    #exit 1
    pwd
    echo merge_unwrap_geocode_tops.csh tmp.filelist $3
    merge_unwrap_geocode_tops.csh tmp.filelist $3
