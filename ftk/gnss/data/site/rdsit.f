...
c     ---
      character*512 bufstr,tmpsites(5000)
      character*10240 bufline
      character*1 sep
      integer*4 fid, i,ioerr
      integer nblen,iargc
c     ---
      if (iargc().lt.1) then
         write(*,*) '[rdsit]Usage: rdsit site_file'
         stop
      endif
...
c      fid=90
      ...
      if (ioerr.ne.0) then
         write(*,'(3a)') '[rdsit]ERROR: cannot open input file "',
     &       file(1:nblen(file)),'".'
         stop
      endif
 800  read(fid,'(a10240)',end=899) bufline
