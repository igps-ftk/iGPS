      SUBROUTINE scargle(x,y,n,ofac,hifac,px,py,np,nout,jmax,prob,nmax)
      INTEGER*4 jmax,n,nout,np,NMAX
      REAL*8 hifac,ofac,prob,px(np),py(np),x(np),y(np)
c      PARAMETER (NMAX=2000)
CU    USES avevar
      INTEGER*4 i,j
      REAL*8 ave,c,cc,cwtau,effm,expy,pnow,pymax,s,ss,sumc,sumcy,sums,
     *sumsh,sumsy,swtau,var,wtau,xave,xdif,xmax,xmin,yy
      DOUBLE PRECISION arg,wtemp,wi(NP),wpi(NP),wpr(NP),wr(NP),
     *TWOPID
