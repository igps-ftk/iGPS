...
#
  ...
  # sample the dem
  if ($mode == 2) then
    echo "creating flt.grd and topo.llt"
    if (! -s flt.grd) gmt grdfilter $2 -D2 -Fg2 -I12s -Gflt.grd
    if (! -s topo.llt) gmt grd2xyz --FORMAT_FLOAT_OUT=%lf flt.grd -s > topo.llt
  endif
  ...
  ### rm  *.PRM* *.SLC *.LED *tmp*
  rm *tmp*
  if (-f baseline_table.dat.upd) rm baseline_table.dat.upd
  ...
  foreach line (`awk '{print $0}' $1`)
    ...
    set m_stem_master = `echo $line | awk -F: '{print $1}' | awk '{ print "S1A"substr($1,16,8)"_ALL_F"substr($1,7,1)"_U_"}'`
     ### BASELINE
    if($mode == 1) then
      echo "NOT done!"
      ####exit 1
      ...
      echo $image
      echo $orbit
      echo $stem_master $m_stem_master
      ###
      ...
      ###
      baseline_table.csh $mmaster.PRM $m_stem_master.PRM >> baseline_table.dat.upd
      baseline_table.csh $mmaster.PRM $m_stem_master.PRM GMT >> table.gmt
      ###
      #### clean up the mess
      rm junk1 junk2
    else if($mode == 2) then ###ALIGNMENT
      ...
      foreach file (`awk '{print $0}' tmp.filelist`)
        ...
        # get images aligned
        if($sl == 1) then
          echo "master should already done, skip it"
          #y##### sl=1 means processing the super_master image, simpley generate images and stitch them together
          #y####make_s1a_tops $file.xml $file.tiff $stem 1
          #y####make_s1a_tops $file.xml $file.tiff $stem 2
          #y####ext_orb_s1a $stem.PRM $orbit $stem
          ...
          #
          #if already done, then skip it
          set stem2 = `echo $line | awk -F: '{print $1}' | awk '{ print "S1A"substr($1,16,8)"_ALL_F"substr($1,7,1)}'`
          if ( -s "${stem2}.SLC" ) then
            echo " already done, skip ${stem2}."
            ###echo $stem >> tmp.stitchlist
            set sl = `echo $sl | awk '{print $1+1}'`
            continue
          endif
          ...
          echo cp $tmp_master.PRM tmp.PRM
    ...
    endif #end-of-if-else-mode-equals-2
  end
