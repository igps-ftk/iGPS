...
; :AUTHOR: tianyf
;-
PRO PROFILE_LINE_AUTO_GET_PROFILE_xy, xy1, xy2, xy3, oxy=oxy, $
    strike=strike,  $
...
; :AUTHOR: tianyf
;-
PRO PROFILE_LINES_AUTO_xy, xys,  $ ;input; mondatory; double [2,npt]; in decimal degrees
  ...
  km_per_deg=1d0  ;MAP_2POINTS(0,latmid,1,latmid,/meters)*1d-3
  ...
  IF auto_strike EQ 2 THEN BEGIN ;no strike from input
    ...
    FOR i=0,N_ELEMENTS(xys[0,*])-1-1 DO BEGIN
      ...
      strike=ATAN(tmp)
      ;
      if (!dpi/2d0-strike)*180/!dpi gt 90 then begin
        strike=strike+!dpi
      endif 
  ...
  FOR i=0,N_ELEMENTS(xys[0,*])-2 DO BEGIN
    ...
    len_seg_i=sqrt( (xy1[0]-xy2[0])^2+(xy1[1]-xy2[1])^2  )
    ...
    IF len_seg_i+len_acc_i GT spacing THEN BEGIN
      IF len_acc_i EQ 0 THEN BEGIN
        ...
        WHILE len_acc_i GT spacing DO BEGIN
          npf=npf+1
          GEO_LINE_XY_FAR_DIST_xy, xy1, xy2, xy3, spacing,oxy=oxy
          PROFILE_LINE_AUTO_GET_PROFILE_xy, xy1, xy2, oxy, oxy=oxy_pf, $
      ...
      ENDIF ELSE BEGIN
        ;STOP
        npf=npf+1
        GEO_LINE_XY_FAR_DIST_xy, xy1, xy2, xy3, spacing-len_acc_i,oxy=oxy
        PROFILE_LINE_AUTO_GET_PROFILE_xy, xy1, xy2, oxy, oxy=oxy_pf, $
          strike=strike, km_per_deg=km_per_deg, isplot=isplot,length_profile=length_profile
        xys_pf=[[xys_pf],[oxy]]
        oxys=[[[oxys]],[[oxy_pf]]]
