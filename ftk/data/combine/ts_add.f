...
c     --Local Parameters--
      integer*4 nmaxf
      parameter(nmaxf=5000)
      character*200 filter,dt,cfilter
      character*1000 files(nmaxf),file,ofile,headers(nmax_head)
...
c     for date conversion
      ...
      character*1024 cfiles(nmaxf),headersc(nmax_head),header
...
c     <<VAR_DEC
       ...
       if (iargc().lt.3) then
         write(*,*) 'Syntax: ts_add path path_to_add opath [TYPE]'
