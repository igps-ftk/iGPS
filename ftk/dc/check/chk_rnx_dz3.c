...
int func_crx2rnx_z(char fname[512]){
  ...
  while( fgets(dline,MAXCLM,ifp) != NULL ){      /*** exit program successfully ***/
    nl_count++;
    CHOP_LF(dline,p);
  SKIP:
    if(crinex_version == 3) { /*** skip escape lines of CRINEX version 3 ***/
      while(dline[0] == '&'){
	nl_count++;
	if( fgets(dline,MAXCLM,ifp) == NULL ) {
	  pclose(ifp);
	  return exit_status;
	}
	CHOP_LF(dline,p);
...
int fileopen(int argc, char *argv[]){
  ...
  sprintf(openstr,"gunzip <  %s",infile);
  if((ifp = popen(openstr,"r")) == NULL) {error_exit(4,infile);return 1;}
...
int  read_chk_line(char *line){
  ...
  if( fgets(line,MAXCLM,ifp) == NULL ) {error_exit(8,line);return 1;}
  //printf("pass here");
  if( (p = strchr(line,'\n')) == NULL) {
    if( fgetc(ifp) == EOF ) {     /** check if EOF is there **/
      error_exit(8,line);
      /*
...
main(int argc, char *argv[]){
  ...
  */
  ...
  for (year=yrs;year<=yre;year++){
	...
	if (ok==0){
	  //printf("OK\n");
	  printf("GOOD:%4s:%d:%03d:d;%s\n",site,year,doy,file);
	}else{
	  //printf("ERROR\n");
	  printf(" BAD:%4s:%d:%03d:d;%s\n",site,year,doy,file);
	}
      }
      closedir(dps);

      /* check in the $DOY/$YYd  sub-directoyr */
      sprintf(dpath,"%s/%03d/%02dd",ypath,doy,yr);
      //printf("%s\n",dpath);
      dps=opendir(dpath);
      if (dps==NULL) continue;
      while(eps=readdir(dps)){
