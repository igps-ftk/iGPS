...
c     <<VAR_DEC
       ...
       if (iargc().lt.2) then
         write(*,*) 'Syntax: sio2otx path opath [TYPE]'
...
c               write(*,*) 'jd ->ymdhms'
               call jd_to_ymdhms(jd,idate,sectag)
               write(fido,700) idate(1),idate(2),idate(3),
     &              data(i,neui+3)*1e3,
     &              data(i,neui+6)*1e3
            enddo
 700        format(i4,1x,i2,1x,i2,1x,f10.5,1x,f10.5)
            close(fido)
c            stop
         enddo
      enddo
