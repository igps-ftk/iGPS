...
c     <<VAR_DEC
       ...
       if (iargc().lt.2) then
          write(*,*) 'Syntax: trk2hneus path opath'
         stop
      endif
      call getarg(1,path)
      call getarg(2,opath)
...
c         ofile=ofile//'.neu'
         ...
         ofile=tmpstr(1:nblen(tmpstr))//'.hneu'
c         write(*,*) ofile(1:nblen(ofile))
...
c     &           idate(1),doy,
     ...
     &           data(i,11),data(i,8),data(i,10),data(i,12)
         enddo
c 700     format(f20.15,1x,i4,1x,i3.3,3f15.5,3f10.5)
 700     format(f20.15,1x,i4,4(1x,i2),1x,f10.7,3f15.5,3f10.5)
         close(fido)
      enddo
