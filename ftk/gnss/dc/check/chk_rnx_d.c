...
int func_crx2rnx(char fname[512]){
...
int fileopen(int argc, char *argv[]){
  ...
  /***********************/
  p=strrchr(infile,'.');
  if(p == NULL || (*(p+3) != 'D' && *(p+3) != 'd') || *(p+4) != '\0') {error_exit(3,p);return 1;}
  if((ifp = fopen(infile,"r")) == NULL) {error_exit(4,infile);return 1;}
...
int  read_chk_line(char *line){
  ...
  nl_count++;
  if( fgets(line,MAXCLM,stdin) == NULL ) {error_exit(8,line);return 1;}
  if( (p = strchr(line,'\n')) == NULL) {
