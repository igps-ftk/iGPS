...
;-
PRO TIME_SERIES_STACKING, PATH=PATH, $
  ...
  IF N_ELEMENTS(PATH) EQ 0 && N_ELEMENTS(FILES) EQ 0 THEN BEGIN
    PATH=FILEPATH(ROOT_DIR=!IGPS_ROOT,SUBDIRECTORY=['example','sio'], $
      'cleanedNeuUnf.resid')
    ;    path='D:\gsar\des\mila2\des_F3\SBAS8\outp\resid'
    ;    cme_site_file='D:\gsar\des\mila2\des_F3\SBAS8\outp\resid\cme.sit'
    ;  ;path='D:\gsar\asc\mila1\asc_F1\SBAS4\nc.detrend\outp'
      
    ;
    ;daxiong co
    path='D:\gsar\asc\xiangyang_boruo1.3\asc_F1\SBAS8\x3\raw'
    path='D:\gsar\asc\xiangyang_boruo1.3\asc_F1\SBAS8.atm3\x3\raw'
    path='D:\gsar\asc\daxionglake\nc\x3\raw'
    ;    path='D:\gsar\des\buruo2\des_F2.d\SBAS.1368f\x3\raw'
    ;    path='D:\gsar\des\buruo2\des_F2.d\SBAS.1379\x3\raw'
    ;    path='D:\gsar\des\buruo2\des_F2.d\SBAS.1681\x3\raw'
    ;    path='D:\gsar\asc\heartlake1\asc_F3\SBAS.1332\x3\raw'
    
    
    ;  path='D:\gsar\asc\co.jieze.20180927.1\asc_F3\SBAS10\x7\raw'
    path='\\10.4.35.82\root\sar\proc_gmtsar\auto_grid\A142_0096\F3\sbas.3.0.0001.0072.20170520.20210616.119.0648.01.___\r4\x1\raw'
    
    path='D:\gsar\auto_grid\A113_0111\F1\sbas.2.0.0001.0072.20210801.20220609.027.0141.01.___\roi_11031\ts'
    path='D:\gsar\interseismic\137-a-m3-0148_0153_0158-cascadia\f123\sbas.4.0.0001.9999.20150107.20221220.168.0585.01.___\r4\x7\raw'
  ...
  IF N_ELEMENTS(ofile) EQ 0 THEN  OFILE=GETPATHNAME(PATH)+PATH_SEP()+'cmes.neu'
  
  IF N_ELEMENTS(sites) EQ 0 THEN BEGIN
    IF N_ELEMENTS(cme_site_file) EQ 0 THEN BEGIN
      SITES=STRMID(GETFILENAME(FILES),0,4)
    ENDIF ELSE BEGIN
      RDSIT,cme_site_file, site=sites
    ENDELSE
  ENDIF
  
  PRINT, '['+PROG+']CME sites:',SITES, $
    FORMAT='(A,10A5)'
  ...
  ;;
  ;;OUTPUT CME SERIES
  PRINT,'['+PROG+']CME series output to ',OFILE, $
    FORMAT='(2A)'
  ...
  CME_APPLY_STACKING_NIKOLAIDIS2002, PATH
END